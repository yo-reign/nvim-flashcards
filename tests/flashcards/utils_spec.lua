describe("utils", function()
  local utils

  before_each(function()
    package.loaded["flashcards.utils"] = nil
    utils = require("flashcards.utils")
  end)

  describe("generate_id", function()
    it("returns 8-char alphanumeric string", function()
      local id = utils.generate_id()
      assert.equals(8, #id)
      assert.truthy(id:match("^[a-z0-9]+$"))
    end)

    it("generates unique ids", function()
      local ids = {}
      for _ = 1, 100 do
        local id = utils.generate_id()
        assert.is_nil(ids[id], "duplicate id: " .. id)
        ids[id] = true
      end
    end)
  end)

  describe("extract_card_id", function()
    it("extracts id from comment", function()
      local id = utils.extract_card_id("some text <!-- fc:abc12345 -->")
      assert.equals("abc12345", id)
    end)

    it("returns nil when no id", function()
      assert.is_nil(utils.extract_card_id("no id here"))
    end)

    it("detects suspended flag", function()
      local id, flags = utils.extract_card_id("text <!-- fc:abc12345 !suspended -->")
      assert.equals("abc12345", id)
      assert.truthy(flags.suspended)
    end)
  end)

  describe("format_card_id", function()
    it("formats id as comment", function()
      assert.equals("<!-- fc:abc12345 -->", utils.format_card_id("abc12345"))
    end)

    it("formats with suspended flag", function()
      assert.equals("<!-- fc:abc12345 !suspended -->",
        utils.format_card_id("abc12345", { suspended = true }))
    end)
  end)

  describe("extract_source_ref", function()
    it("extracts a leading section reference", function()
      local ref, text = utils.extract_source_ref("(1.2.3:5) What is X?")
      assert.equals("1.2.3:5", ref)
      assert.equals("What is X?", text)
    end)

    it("extracts paragraph ranges", function()
      local ref, text = utils.extract_source_ref("(1.1.2:8-10) Name the sets")
      assert.equals("1.1.2:8-10", ref)
      assert.equals("Name the sets", text)
    end)

    it("strips marker-only first lines from multiline fronts", function()
      local ref, text = utils.extract_source_ref("(1.1.2:8-10)\nName the sets")
      assert.equals("1.1.2:8-10", ref)
      assert.equals("Name the sets", text)
    end)

    it("leaves ordinary parenthesized prompts intact", function()
      local ref, text = utils.extract_source_ref("(optional) What is X?")
      assert.is_nil(ref)
      assert.equals("(optional) What is X?", text)
    end)

    it("extracts explicit free-form source refs", function()
      local ref, text = utils.extract_source_ref("(ref: Lang, Algebra I §2) Define a group")
      assert.equals("Lang, Algebra I §2", ref)
      assert.equals("Define a group", text)
    end)

    it("keeps legacy arbitrary-prefix parsing available for scanner migration", function()
      local ref, text = utils.extract_legacy_source_ref("(Lang, Algebra I §2) Define a group")
      assert.equals("Lang, Algebra I §2", ref)
      assert.equals("Define a group", text)
    end)
  end)

  describe("expand_template_vars", function()
    it("expands file.name", function()
      local result = utils.expand_template_vars("{{file.name}}", "math/algebra.md", "notes/")
      assert.equals("algebra", result)
    end)

    it("expands file.dir", function()
      local result = utils.expand_template_vars("{{file.dir}}", "math/algebra.md", "notes/")
      assert.equals("math", result)
    end)

    it("expands file.path", function()
      local result = utils.expand_template_vars("{{file.path}}", "math/sub/algebra.md", "notes/")
      assert.equals("math/sub/algebra", result)
    end)

    it("handles no template vars", function()
      local result = utils.expand_template_vars("plain text", "math/algebra.md", "notes/")
      assert.equals("plain text", result)
    end)
  end)

  describe("parse_tags", function()
    it("extracts tags from text", function()
      local tags = utils.parse_tags("some text #math #algebra/linear")
      assert.same({ "math", "algebra/linear" }, tags)
    end)

    it("returns empty for no tags", function()
      assert.same({}, utils.parse_tags("no tags here"))
    end)

    it("ignores # inside inline code spans", function()
      local tags = utils.parse_tags("uses `#define FOO 1` for macros #c")
      assert.same({ "c" }, tags)
    end)

    it("ignores multiple code spans with #", function()
      local tags = utils.parse_tags("`#include <stdio.h>` and `#pragma once` #c/preprocessor")
      assert.same({ "c/preprocessor" }, tags)
    end)
  end)

  describe("strip_tags", function()
    it("removes tags from text", function()
      assert.equals("some text", utils.strip_tags("some text #math #algebra"))
    end)

    it("preserves inline code containing #", function()
      assert.equals("uses `#define FOO 1` for macros", utils.strip_tags("uses `#define FOO 1` for macros #c"))
    end)

    it("preserves multiple code spans with #", function()
      assert.equals("`#include <stdio.h>` and `#pragma once`",
        utils.strip_tags("`#include <stdio.h>` and `#pragma once` #c"))
    end)
  end)

  describe("trim", function()
    it("trims whitespace", function()
      assert.equals("hello", utils.trim("  hello  "))
    end)
  end)

  describe("trim_display_text", function()
    it("trims leading and trailing spaces from inline card text", function()
      assert.equals("what is x", utils.trim_display_text("what is x "))
      assert.equals("x is a variable.", utils.trim_display_text(" x is a variable."))
    end)

    it("preserves multiline indentation while removing blank outer lines", function()
      local input = "\n    if x then\n      return x\n    end\n"
      assert.equals("    if x then\n      return x\n    end", utils.trim_display_text(input))
    end)
  end)

  describe("normalize_path", function()
    it("expands tilde", function()
      local home = os.getenv("HOME")
      assert.equals(home .. "/notes", utils.normalize_path("~/notes"))
    end)

    it("removes trailing slash", function()
      assert.equals("/home/user/notes", utils.normalize_path("/home/user/notes/"))
    end)
  end)

  describe("card path resolution", function()
    it("resolves canonical and legacy paths inside configured roots", function()
      local root = vim.fn.tempname()
      vim.fn.mkdir(root, "p")
      local path = root .. "/card.md"
      assert.is_true(utils.write_file(path, "Q ::: A"))

      local canonical = utils.canonical_path(path)
      assert.equals(canonical, utils.resolve_card_path(canonical, { root }))
      assert.equals(canonical, utils.resolve_card_path("card.md", { root }))
      assert.is_nil(utils.resolve_card_path("../card.md", { root }))

      os.remove(path)
      os.remove(root)
    end)
  end)

  describe("time helpers", function()
    it("now returns unix timestamp", function()
      local t = utils.now()
      assert.is_number(t)
      assert.truthy(t > 1700000000)
    end)

    it("format_interval shows human readable", function()
      assert.equals("< 1m", utils.format_interval(0))
      assert.equals("10m", utils.format_interval(0.007))
      assert.equals("1h", utils.format_interval(0.04))
      assert.equals("1d", utils.format_interval(1))
      assert.equals("7d", utils.format_interval(7))
      assert.equals("1.0mo", utils.format_interval(30))
      assert.equals("1.0y", utils.format_interval(365))
    end)
  end)
end)
