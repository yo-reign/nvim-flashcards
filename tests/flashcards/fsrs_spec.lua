...
-- Run with: nvim --headless -c "PlenaryBustedFile tests/flashcards/fsrs_spec.lua" -c "qa"
...
describe("fsrs", function()
    ...
    describe("FSRS:new", function()
        ...
        it("should merge custom weights with defaults", function()
            local fsrs = require("flashcards.fsrs")
            local scheduler = fsrs.new({
                weights = { initial_stability_correct = 5.0 },
            })

            assert.equals(5.0, scheduler.weights.initial_stability_correct)
            -- Other defaults preserved
            assert.equals(0.5, scheduler.weights.initial_stability_wrong)
            assert.equals(5.0, scheduler.weights.initial_difficulty)
        end)
    end)
